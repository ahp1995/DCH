...
def save_cifar10_png_images(data_path):
    os.mkdir(data_path + '/cifar10_image')
    mean = [0.4914, 0.4822, 0.4465]
    std = [0.2023, 0.1994, 0.2010]
    transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
    cifar10_image = datasets.CIFAR10(data_path, train=True, download=True, transform=transform) # no augmentation
    
    for i in range(len(cifar10_image)): # get CIFAR10 image and convert to png file for HPF image
        img = cifar10_image[i][0]
        save_image(img, data_path + '/cifar10_image/img{}.png'.format(i))
    print('Save CIFAR10 images are ended.')
    
def save_cifar100_png_images(data_path):
    os.mkdir(data_path + '/cifar100_image')
    mean = [0.5071, 0.4866, 0.4409]
    std = [0.2673, 0.2564, 0.2762]
    transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
    cifar100_image = datasets.CIFAR100(data_path, train=True, download=True, transform=transform) # no augmentation
    
    for i in range(len(cifar100_image)): # get CIFAR100 image and convert to png file for HPF image
        img = cifar100_image[i][0]
        save_image(img, data_path + '/cifar100_image/img{}.png'.format(i))
    print('Save CIFAR100 images are ended.')
    
def get_hpf_dataset(data_path, filter, dataset):
    if filter == 'dogf':
        if dataset == 'CIFAR10':
            mean = [0.3137, 0.3137, 0.3137]
            std = [0.4287, 0.4287, 0.4287]
            transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
            dst_train_hpf = torchvision.datasets.ImageFolder(root=data_path + "/cifar10_dogf", transform = transform)
        elif dataset == 'CIFAR100':
            mean = [0.3053, 0.3053, 0.3053]
            std = [0.4264, 0.4264, 0.4264]
            transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
            dst_train_hpf = torchvision.datasets.ImageFolder(root=data_path + "/cifar100_dogf", transform = transform)
        else:
